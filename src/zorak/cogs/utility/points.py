    ...
    async def leaderboard(self, ctx):
        ...
        def is_staff(member_obj):
            ...
            for role in settings.admin_roles:
                if settings.admin_roles[role] in [y.id for y in member_obj.roles]:
                    return True
            return False

        top10_no_staff = []
        points = self.bot.db_client.get_top_10()
        guild = self.bot.get_guild(settings.info['id'])
        ...
        embed = embed_leaderboard(top10_no_staff, settings.info['name'],
                                  settings.info['logo'])
