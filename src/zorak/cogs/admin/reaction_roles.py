...
class RoleDropdownSelector(discord.ui.Select):
    ...
    def __init__(self, selector_data):
        ...
        options.append(discord.SelectOption(label="None", description="Remove all roles", emoji="❌", value="0"))
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        roles = [discord.utils.get(interaction.guild.roles, id=option["id"]) for option in self.reaction_roles]
        ...
        if selected_role is not None or selection == "0":
            if selection == "0":
                ...
                await interaction.response.send_message("Removed all roles in this group!", ephemeral=True)
            else:
                if selected_role.id in interaction.user.roles:
                    await interaction.user.remove_roles(selected_role)
                    await interaction.response.send_message(f"Removed the <@&{selected_role.id}> role!", ephemeral=True)
                else:
                    if self.single_choice:
                        await remove_roles_if_exists(interaction.user, roles)
                    await interaction.user.add_roles(selected_role)
                    await interaction.response.send_message(f"Assigned the <@&{selected_role.id}> role!", ephemeral=True)
        else:
...
class Roles(commands.Cog):
    ...
    async def roles(self, ctx):
        ...
        if hasattr(self.bot.server_settings, "reaction_role_data"):
