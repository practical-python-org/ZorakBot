    ...
    async def purge_messages(self, ctx, number_messages):
        ...
        if ctx.channel.id in log_channel.values():
            await ctx.channel.send(
                f"{ctx.author.mention}, you should not be purging the {ctx.channel.mention}."
            )
            await logs_channel.send(
                f"{ctx.author.mention} atempted to purge the {ctx.channel.mention}."
            )
        else:
            # Do the purge
    ...
    async def cog_command_error(
        ...
        if isinstance(error, commands.MissingPermissions):
            await ctx.channel.send(
                ...
                f"correct permissions to use this command!",
                reference=ctx.message,
            )

        if isinstance(error, commands.MissingRole):
            await ctx.channel.send(
                f"Sorry, {ctx.author.name}, you must be a"
                f" memeber of Staff to use this command!",
                reference=ctx.message,
            )
