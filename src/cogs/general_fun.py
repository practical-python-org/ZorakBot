...
class Fun(commands.Cog):
    ...
    async def pugfact(self, ctx):
        await ctx.respond(
            BeautifulSoup(
                requests.get("https://fungenerators.com/random/facts/dogs/pug").content,
                "html.parser",
            )
            .find("h2")
            .text[:-15]
    ...
    async def catpic(self, ctx):
        await ctx.respond(
            file=discord.File(
                fp=BytesIO(requests.get("https://cataas.com/cat").content),
                filename="cat.png",
            )
        )
    ...
    async def dogpic(self, ctx, *, breed=None):
        ...
        if breed is None:
            link = requests.get("https://dog.ceo/api/breeds/image/random").json()[
                "message"
            ]
    ...
    async def pokedex(self, ctx, pokemon: discord.Option(str)):
        ...
        if data.status_code == 200:
            ...
            embed.set_thumbnail(
                url="https://assets.pokemon.com/assets/cms2/img/misc/gus/buttons/logo-pokemon-79x45.png"
            )
