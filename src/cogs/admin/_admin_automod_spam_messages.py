            ...
            for channel in guild.text_channels:
                async for last_message in channel.history(limit=10, after=time_ago):
                    if last_message.content == message_in_question.content:
                        if last_message.author == message_in_question.author\
                                and message_in_question.author.bot is False:

                            await last_message.channel.send(
                                f"you already posted this in {message_in_question.channel.mention}")
