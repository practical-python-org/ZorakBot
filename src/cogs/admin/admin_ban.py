            ...
            if not target.guild_permissions.administrator:
                ...
                await ctx.respond(
                    embed=embed_cant_do_that(
                        f"**{ctx.author.name}** banned **{target.name}**"
                        f"\n**Reason:** {reason}")
                )

            else:
                await ctx.respond(
                    embed=embed_cant_do_that("You can't ban an Admin.")
                    , ephemeral=True)
        else:
            await ctx.respond(
                embed=embed_cant_do_that("You cant ban a bot.")
                , ephemeral=True)
