...
from ._settings import RSS_Feed, normal_channel


class RSS_feeds(commands.Cog):
		...
		def send_news(ID_to_send):
			...
			def send_jetbrains(ID_tag):
				...
				if ID_tag == id_code:
					...
					embed=discord.Embed(title=f"**{NewsFeed['entries'][0]['title']}**"
