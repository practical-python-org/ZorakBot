...
class BlackjackView(discord.ui.View):
    ...
    async def hit_button(self,button:discord.ui.Button,interaction:discord.Interaction):
        ...
        if self.game.player.score > 21:
            for child in self.children:
                child.disabled=True
                await interaction.response.edit_message(f"You drew a {str(self.game.player.hand[-1])} and busted with {str(self.game.player.score)}!",view=self)
        await interaction.response.edit_message(f"You drew a {str(self.game.player.hand[-1])}. You're on {str(self.game.player.score)}. Hit or Stand?", view=self)
    ...
    async def stand_button(self,button:discord.ui.Button,interaction:discord.Interaction):
        ...
        if self.game.dealer.score > 21:
            ...
            await interaction.response.edit_message(f"Dealer wins with {str(self.game.dealer.score)} against your {str(self.game.player.score)}.", view=self)
        elif self.game.dealer.score < self.game.player.score:
